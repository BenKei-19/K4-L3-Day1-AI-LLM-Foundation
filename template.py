...
def call_openai_mini(
    ...
    return call_openai(prompt=prompt, model=OPENAI_MINI_MODEL, top_p=top_p, temperature=temperature, max_tokens=max_tokens)
...
def compare_models(prompt: str) -> dict:
    ...
    """
    gpt4o_res, gpt4o_latency = call_openai(prompt)
    mini_res, mini_latency = call_openai_mini(prompt)
    gpt4o_cost = (len(gpt4o_res.split()) / 0.75) / 1000 * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
    return {
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    res = ["Prompt | GPT-4o Response | Mini Response | GPT-4o Latency | Mini Latency"]
    for r in results:
        res.append(
            f"{r['prompt']} | {r['gpt4o_response']} | {r['mini_response']} | {r['gpt4o_latency']} | {r['mini_latency']}"
        )
    return "\n".join(res)
